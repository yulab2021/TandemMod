...
def extract_signal(file_list,read_id_dict):
	...
	if int(args.process) > 1:
		...
		for result in results:
			...
			if num[0]:
				with open(args.output+"_%s" %pid, 'a') as out:
...
def main():
    ...
    read_id_dict=get_read_id_dict("/data/wuyou/ac4c/data/col0_DRS_3_guppy/sequencing_summary.txt")
